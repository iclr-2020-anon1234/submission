...
This contains the code to reproduce the experiments in our paper.
The main model is located in the `fspool.py` file in this top-level directory.

The polygon auto-encoder, MNIST auto-encoder, and MNIST classification experiments are located in the `auto-encoder` directory.
The CLEVR classification experiment is located in the `CLEVR` directory.
The graph classification experiment is located in the `graphs` directory.
The CLEVR set prediction experiment is located in the `dspn` directory.

Note that because of differing PyTorch versions when running these experiments, the `fspool.py` files in these directories may be slightly different, though should be functionally-equivalent to the main `fspool.py`.
The only difference is in the initialisation, see experimental details in the supplementary material for details about this.
